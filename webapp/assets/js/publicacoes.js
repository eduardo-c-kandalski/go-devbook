$("#nova-publicacao").on("submit", criaPublicacao);
...
function criaPublicacao(evento) {
...
function curtirPublicacao(evento) {
  ...
  $.ajax({
    ...
    .done(function () {
      const contatorDeCurtidas = elementoClicado.next("span");
      const quantidadeDeCurtidas = parseInt(contatorDeCurtidas.text());

      contatorDeCurtidas.text(quantidadeDeCurtidas + 1);
...
function atualizarPublicacao(evento) {
  ...
  $.ajax({
    ...
    .done(function () {
      Swal.fire("Sucesso", "Publicação editada com sucesso", "success").then(
        function () {
          window.location = "/home";
        }
      );
